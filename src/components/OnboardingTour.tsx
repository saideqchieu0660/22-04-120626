        ...
        <motion.div 
         ...
         >
           <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white dark:bg-zinc-900 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative border border-stone-200 dark:border-zinc-800"
            >
               <button onClick={handleSkipOrFinish} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 bg-black/5 dark:bg-white/5 rounded-full p-1.5 transition">
                 <X className="w-5 h-5" />
               </button>
               
               <div className="mb-6">
                  <div className="flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-500 text-[10px] font-bold uppercase tracking-wider rounded-lg px-2.5 py-1 w-fit mb-4">
                     <Compass className="w-3.5 h-3.5" />
                     Hướng dẫn người mới ({currentStep + 1}/{TOUR_STEPS.length})
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold mb-3 font-display tracking-tight text-stone-900 dark:text-stone-100">{step.title}</h3>
                  <p className="text-sm text-stone-600 dark:text-stone-400 leading-relaxed">
                    {step.content}
                  </p>
               </div>

               <div className="flex flex-col sm:flex-row items-center justify-end mt-8 border-t border-stone-100 dark:border-zinc-800 pt-5 gap-4">
                  <div className="flex items-center justify-end w-full sm:w-auto gap-2">
                     <button 
                         onClick={handleSkipOrFinish}
                         className="p-2 md:px-3 rounded-xl border border-transparent hover:bg-stone-100 dark:hover:bg-zinc-800 text-stone-500 dark:text-stone-400 font-medium transition text-sm h-10"
                     >
                        Bỏ qua
                     </button>
                     {currentStep > 0 && (
                        <button 
                          ...
                          className="p-2 md:px-3 rounded-xl border border-stone-200 dark:border-zinc-700 hover:bg-stone-50 dark:hover:bg-zinc-800 text-stone-600 dark:text-stone-300 transition flex items-center justify-center font-medium shadow-sm h-10 min-w-[40px] text-sm"
                        >
                     ...
                     <button 
                       ...
                       className="flex items-center justify-center gap-2 bg-stone-900 dark:bg-amber-500 text-white dark:text-stone-900 px-4 rounded-xl text-sm font-bold hover:opacity-90 transition shadow-md h-10 w-full sm:w-auto"
                     >

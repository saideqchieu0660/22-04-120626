              ...
              {manualMode === "form" ? (
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="flex justify-between items-center">
                    <h4 className="text-sm font-black uppercase opacity-70 block tracking-wide">Nhập từ vựng & nghĩa thủ công từng thẻ</h4>
                    <span className="text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold px-2 py-0.5 rounded">Form trực tiếp</span>
                  </div>

                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1.5 scrollbar-thin">
                    {manualRows.map((row, idx) => (
                      <div key={idx} className="flex gap-3 items-center animate-in slide-in-from-top-1 duration-150">
                        <span className="text-xs opacity-55 font-mono w-5 shrink-0 text-center">{idx + 1}</span>
                        ...
                        {manualRows.length > 1 && (
                          <button
                            type="button"
                            onClick={() => {
                              setManualRows(manualRows.filter((_, rIdx) => rIdx !== idx));
                            }}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      type="button"
                      onClick={() => setManualRows([...manualRows, { front: "", back: "" }])}
                    ...
                    <button
                      type="button"
                      disabled={manualRows.filter(r => r.front.trim() && r.back.trim()).length === 0}
                      onClick={() => {
                        const validRows = manualRows.filter(row => row.front.trim() && row.back.trim());
                    ...
                    >
                      Xác nhận & Chuyển sang Nạp ({manualRows.filter(r => r.front.trim() && r.back.trim()).length} thẻ)
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4 animate-in fade-in duration-200">
                  ...
                  <div className="flex gap-4 items-center justify-center pt-2 w-full">
                    ...
                    >
                      {isAiParsingJson ? (
                        <>
                          <Loader2 className={cn("w-5 h-5", !isEcoMode && "animate-spin")} /> AI Đang Sửa Lỗi Cú Syntax...
                        </>
                      ) : (
                        <>
